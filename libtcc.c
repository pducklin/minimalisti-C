...
#else
/* here we are using the binary as the root of the supporting files */
static void tcc_set_lib_path_selfexe(TCCState *s)
{
    char path[4096], *p;
    memset(path,0,sizeof(path)); /* readlink does not null terminate! */
    if (readlink("/proc/self/exe", path, sizeof(path)-1) == -1) {
        tcc_error("can't find path of running binary");
    }
    p = tcc_basename(path);
    if (p > path)
        --p;
...
# ifdef _WIN32
    if (!s->nosysdir && output_type != TCC_OUTPUT_OBJ)
        tcc_add_systemdir(s);
# endif
    return 0;
}
...
reparse:
        ...
        case TCC_OPTION_unwind:
            s->pe_unwind = 1;
            break;    
        case TCC_OPTION_nounwind:
            s->pe_unwind = 0;
            break;    
        case TCC_OPTION_easyfa:
            s->pe_file_align = s->section_align;
            break;
        case TCC_OPTION_aslr:
            s->pe_aslr = 1; 
            break;
        case TCC_OPTION_dep:
            s->pe_dep = 1;
