 ...
 */
 ...
 /* £££ Added more recent headers to include memmove_s() and others (for PETCC64 and luax */
...
extern "C" {
  ...
  _CRTIMP errno_t __cdecl strcat_s(char *_Dst, size_t _SizeInBytes, const char * _Src);

  // __inline size_t __cdecl strnlen_s(const char * _src, size_t _count) { return _src ? strnlen(_src, _count) : 0; }
...
#define _WSTRING_S_DEFINED
  ...
  _CRTIMP errno_t __cdecl wcscpy_s(wchar_t *_Dst, size_t _SizeInWords, const wchar_t *_Src);
  _CRTIMP errno_t __cdecl wcscat_s(wchar_t * _Dst, size_t _SizeInWords, const wchar_t *_Src);
  ...
  //__inline size_t __cdecl wcsnlen_s(const wchar_t * _src, size_t _count) { return _src ? wcsnlen(_src, _count) : 0; }
#endif
