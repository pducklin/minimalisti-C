...
static int pe_assign_addresses (struct pe_info *pe)
{
    ...
    /* We make .relocs for all files, so that /DYNAMICBASE is possible */
...
static void pe_add_runtime(TCCState *s1, struct pe_info *pe)
{
    ...
    if (find_elf_sym(symtab_section, "wWinMain")) {
        pe_type = PE_GUI;
    ...
    /* what's the internal entry point name? */
    if (PE_DLL == pe_type) {
        start_symbol = "__dllstart";
    } else 
    if (PE_GUI == pe_type) {
        start_symbol = unicode_entry ? "__wwinstart": "__winstart";
    } else {
        start_symbol = unicode_entry ? "__wstart" : "__start";
    }  
    ...
    if (0 == s1->nostdlib) {
        ...
        for (pp = libs; 0 != (p = *pp); ++pp) {
            if (0 == *p) {
                if (PE_DLL != pe_type && PE_GUI != pe_type && s1->pe_subsystem != 2)
                    break;
...
static void pe_set_options(TCCState * s1, struct pe_info *pe)
{
    ...
    if (pe->subsystem == 1) {
        pe->section_align = 0x1000;  /* Stick to regular value (was 0x20) */
        pe->file_align = 0x200;      /* Stick to regular value (was 0x20) */
    } else {
...
ST_FUNC int pe_output_file(TCCState *s1, const char *filename)
{
    ...
    if (ret)
        ...
        tcc_error("cannot compile to memory");
    }
